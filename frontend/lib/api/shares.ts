...
export async function getSharesForResource(
  ...
  try {
    ...
    const response = await apiGet<GetSharesResponse>(
      `/api/shares/resource/${resourceType}/${resourceId}`,
    );
...
export async function getEmbedTokenStats(tokenId: string): Promise<EmbedTokenWithStats> {
  try {
    logger.debug("embed_token_stats_fetch_start", "Fetching embed token stats", {
      token_id: tokenId,
    });
