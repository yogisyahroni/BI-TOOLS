...
export const alertsApi = {
  ...
  getHistory: async (
    ...
    const res = await fetchWithAuth(`${API_BASE}/alerts/${id}/history?${params.toString()}`);
    if (!res.ok) {
      const error = await res.json();
  ...
  acknowledge: async (id: string, data?: AcknowledgeAlertRequest): Promise<Alert> => {
    const res = await fetchWithAuth(`${API_BASE}/alerts/${id}/acknowledge`, {
      method: "POST",
      body: JSON.stringify(data || {}),
    });
    if (!res.ok) {
  ...
  mute: async (id: string, data?: MuteAlertRequest): Promise<Alert> => {
    const res = await fetchWithAuth(`${API_BASE}/alerts/${id}/mute`, {
      method: "POST",
      body: JSON.stringify(data || {}),
    });
    if (!res.ok) {
  ...
  unmute: async (id: string): Promise<Alert> => {
    const res = await fetchWithAuth(`${API_BASE}/alerts/${id}/unmute`, {
      method: "POST",
    });
    if (!res.ok) {
      const error = await res.json();
  ...
  getTriggered: async (): Promise<{ alerts: TriggeredAlert[] }> => {
    const res = await fetchWithAuth(`${API_BASE}/alerts/triggered`);
    if (!res.ok) {
      const error = await res.json();
  ...
  getStats: async (): Promise<AlertStats> => {
    const res = await fetchWithAuth(`${API_BASE}/alerts/stats`);
    if (!res.ok) {
      const error = await res.json();
  ...
  triggerCheck: async (id: string): Promise<TriggerAlertCheckResponse> => {
    const res = await fetchWithAuth(`${API_BASE}/alerts/${id}/execute`, {
      method: "POST",
    });
    if (!res.ok) {
      const error = await res.json();
  ...
  test: async (data: TestAlertRequest): Promise<TestAlertResponse> => {
    const res = await fetchWithAuth(`${API_BASE}/alerts/test`, {
      method: "POST",
      body: JSON.stringify(data),
    });
    if (!res.ok) {
