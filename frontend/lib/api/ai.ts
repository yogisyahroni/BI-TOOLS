...
export const aiApi = {
    ...
    // AI Reasoning
    reason: async (data: AIReasoningRequest): Promise<AIReasoningResponse> => {
        ...
        if (!res.ok) {
            const error = await res.json();
            throw new Error(error.error || 'Failed to reason');
        }
        return res.json();
    },

    // AI Optimization
    optimize: async (data: AIOptimizationRequest): Promise<AIOptimizationResponse> => {
