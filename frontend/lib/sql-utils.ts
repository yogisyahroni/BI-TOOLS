...
export function applyFiltersToSql(
    ...
    // Check if filters is array (Cross-Filter) or object (Legacy/Variables)
    if (Array.isArray(filters)) {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ...
            switch (filter.operator) {
                case 'equals':
                    conditions.push(`${fieldName} = ${safeValue(filter.value)}`);
                    break;
                case 'not_equals':
                    conditions.push(`${fieldName} != ${safeValue(filter.value)}`);
                    break;
                ...
                case 'less_than':
                    ...
                    break;
                case 'contains':
                    conditions.push(`${fieldName} ILIKE '%${String(filter.value).replace(/'/g, "''")}%'`);
                    break;
                case 'starts_with':
                    conditions.push(`${fieldName} ILIKE '${String(filter.value).replace(/'/g, "''")}%'`);
                    break;
                ...
                case 'in':
                    if (Array.isArray(filter.value) && filter.value.length > 0) {
                        const values = filter.value.map(safeValue).join(', ');
                        conditions.push(`${fieldName} IN (${values})`);
                    }
                    break;
                case 'between':
                    if (Array.isArray(filter.value) && filter.value.length === 2) {
                        conditions.push(`${fieldName} BETWEEN ${safeValue(filter.value[0])} AND ${safeValue(filter.value[1])}`);
                    }
                    break;
            }
        });
        ...
        if (Object.keys(filters).length === 0) return sql;
...
import type { FilterCriteria } from './cross-filter-context';
