...
export class AlertService {
  ...
  private async evaluateAlert(alert: AlertRule): Promise<boolean> {
    ...
    const _effectiveKey =
      key === "count_*" ? "count_*" : key.toUpperCase() === key ? key : key.toLowerCase(); // Check casing? Aggregation usually returns keys as asked or lowercase.
    ...
    console.warn(
      `[AlertService] Checking ${alert.name}: ${value} ${alert.operator} ${alert.threshold}`,
    );

    switch (alert.operator) {
      case ">":
        return Number(value) > alert.threshold;
      case "<":
        return Number(value) < alert.threshold;
      case "=":
        return Number(value) === alert.threshold;
  ...
  private async sendNotification(alert: AlertRule) {
    ...
    // Audit Logic
    auditService.log(
      { tenantId: "system", userId: "alert-bot", role: "admin" },
      {
