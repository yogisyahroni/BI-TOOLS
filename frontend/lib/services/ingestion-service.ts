...
export class IngestionService {
    ...
    try {
      ...
      if (dbUrl && (!dbConfig.password || dbConfig.password === "")) {
        try {
          ...
          console.warn(
            "[IngestionService] Failed to parse DATABASE_URL, falling back to individual vars",
            e,
          );
        }
      }
  ...
  private static async parseFile(buffer: Buffer, fileType: "xlsx" | "csv"): Promise<any[]> {
    return fileType === "xlsx" ? this.parseExcel(buffer) : this.parseCSV(buffer.toString());
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  }
  ...
  private static async batchInsert(connector: PostgresConnector, tableName: string, data: any[]) {
    ...
    for (let i = 0; i < data.length; i += chunkSize) {
      ...
      const columns = headers
        .map((h) => `"${h.replace(/[^a-zA-Z0-9]/g, "_").toLowerCase()}"`)
        .join(", ");
