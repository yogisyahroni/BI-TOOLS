...
const SOURCE_TYPES = [
  ...
  },
  {
    ...
    emoji: "🌐",
    description: "Fetch from REST endpoint",
  },
];
...
const MODE_OPTIONS = [
  {
    value: "ETL",
    label: "ETL",
    description: "Extract → Transform → Load. Best for cleaning/filtering before storage.",
  },
  {
    value: "ELT",
    label: "ELT",
    description: "Extract → Load → Transform. Best for large datasets with in-DB transforms.",
  },
];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-zinc-950 to-black">
      ...
      <form
        ...
        className="max-w-3xl mx-auto px-6 pb-20 space-y-6"
      >
        {/* Section 0: Basics */}
        {activeSection === 0 && (
          <div className="rounded-xl border border-white/[0.06] bg-zinc-900/50 overflow-hidden">
            <div className="p-6 space-y-5">
              <div>
                <label className="block text-xs font-semibold text-zinc-300 mb-2 tracking-tight">
                  Pipeline Name
                </label>
                <input
                  {...form.register("name")}
                  ...
                  render={({ field }) => (
                    <div className="grid grid-cols-2 gap-3">
                      {MODE_OPTIONS.map((opt) => (
                        ...
                        >
                          <p
                            className={`text-xs font-semibold mb-1 ${field.value === opt.value ? "text-violet-300" : "text-zinc-300"}`}
                          >
                            {opt.label}
                          </p>
              ...
              <div>
                ...
                <p className="text-[10px] text-zinc-600 mt-1">
                  Maximum rows per execution (safety limit)
                </p>
              </div>
            </div>
          </div>
        ...
        {activeSection === 1 && (
          <div className="space-y-4">
            ...
            <div className="rounded-xl border border-white/[0.06] bg-zinc-900/50 p-6">
              <label className="block text-xs font-semibold text-zinc-300 mb-3 tracking-tight">
                Source Type
              </label>
              <Controller
                control={form.control}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {SOURCE_TYPES.map((src) => (
                      ...
                      >
                        ...
                        <div>
                          <p
                            className={`text-xs font-semibold ${field.value === src.value ? "text-violet-300" : "text-zinc-300"}`}
                          >
                            {src.label}
                          </p>
                      ...
                      render={({ field }) => (
                        <div className="space-y-2">
                          {filteredConnections.map((conn) => (
                            ...
                            >
                              <Database
                                className={`w-4 h-4 flex-shrink-0 ${field.value === conn.id ? "text-emerald-400" : "text-zinc-500"}`}
                              />
            ...
            {sourceType === "REST_API" && (
              <div className="rounded-xl border border-white/[0.06] bg-zinc-900/50 p-6">
                ...
                <input
                  ...
                  onChange={(e) =>
                    form.setValue("sourceConfig", JSON.stringify({ endpoint: e.target.value }))
                  }
              ...
              <Controller
                ...
                render={({ field }) => (
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                      <p
                        className={`text-xs font-semibold mb-1 ${field.value === "INTERNAL_RAW" ? "text-emerald-300" : "text-zinc-300"}`}
                      >
                        Internal Storage
                      </p>
                    ...
                    >
                      <p
                        className={`text-xs font-semibold mb-1 ${field.value === "EXTERNAL" ? "text-emerald-300" : "text-zinc-300"}`}
                      >
                        External Database
                      </p>
                      <p className="text-[10px] text-zinc-500">
                        Push to external DB via connection
                      </p>
                    </button>
                  </div>
                )}
