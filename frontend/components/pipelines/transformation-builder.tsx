  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2">
        <Button type="button" variant="secondary" size="sm" onClick={() => addStep("CAST")}>
          + Cast Type
        </Button>
        <Button type="button" variant="secondary" size="sm" onClick={() => addStep("FILTER")}>
          + Filter Row
        </Button>
        <Button type="button" variant="secondary" size="sm" onClick={() => addStep("DROP")}>
          + Drop Col
        </Button>
        <Button type="button" variant="secondary" size="sm" onClick={() => addStep("DEDUPLICATE")}>
          + Dedupe
        </Button>
        <Button type="button" variant="secondary" size="sm" onClick={() => addStep("REPLACE")}>
          + Replace
        </Button>
        <Button
          type="button"
            ...
            <CardContent className="px-4 pb-3 pt-0 text-sm space-y-2">
              ...
              {step.type === "CAST" && (
                <div className="flex gap-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => updateStep(index, "targetType", v)}
                  >
              ...
              {step.type === "FILTER" && (
                <div className="flex gap-2 items-center">
                  ...
                  <Select
                    value={step.operator}
                    onValueChange={(v) => updateStep(index, "operator", v)}
                  >
                    <SelectTrigger className="h-8 w-1/4">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
              ...
              {step.type === "DEDUPLICATE" && (
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Combine checks e.g. email, status (A row is duplicate if ALL match)
                  </p>
                </div>
              )}
              ...
              {step.type === "REPLACE" && (
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => updateStep(index, "replacement", e.target.value)}
                    className="h-8"
                  />
                </div>
              )}
