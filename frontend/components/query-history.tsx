...
const SAMPLE_HISTORY: QueryHistoryEntry[] = [
  ...
  },
  {
    id: "2",
    query:
      "SELECT customer_name, SUM(amount) as total FROM orders WHERE DATE(created_at) >= DATE_TRUNC(...)",
    type: "sql",
      ...
      <div className="absolute right-0 top-0 h-full w-96 bg-card border-l border-border flex flex-col shadow-lg">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3">
          {SAMPLE_HISTORY.map((entry) => (
            <Card
              key={entry.id}
              className="p-3 hover:bg-muted/50 transition-colors cursor-pointer border border-border"
            >
              ...
              <div className="flex items-center justify-between gap-2 text-xs text-muted-foreground">
                <span>
                  {entry.rowsReturned} rows • {entry.duration}ms
                </span>
