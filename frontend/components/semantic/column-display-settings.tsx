...
export function ColumnDisplaySettings({
  ...
  const updateSetting = (
    tableName: string,
    columnName: string,
    updates: Partial<ColumnDisplaySetting>,
  ) => {
    setLocalSettings((prev) => {
      const existing = prev.find((s) => s.tableName === tableName && s.columnName === columnName);
      if (existing) {
        return prev.map((s) =>
          s.tableName === tableName && s.columnName === columnName ? { ...s, ...updates } : s,
        );
      } else {
        return [
          ...prev,
          { tableName, columnName, displayName: columnName, isHidden: false, ...updates },
        ];
      }
    });
  };
