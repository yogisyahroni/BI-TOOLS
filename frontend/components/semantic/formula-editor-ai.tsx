...
import { Sparkles, Loader2, Copy, Check, AlertCircle, CheckCircle2, Plus, X } from "lucide-react";
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Describe the calculation you need</p>
        </div>
      </div>

      {/* Description Input */}
      <div className="flex flex-col gap-2">
        <label className="text-xs font-medium text-muted-foreground">Formula Description</label>
        <Textarea
      ...
      {/* Available Columns */}
      <div className="flex flex-col gap-2">
        <label className="text-xs font-medium text-muted-foreground">Available Columns</label>
        <div className="flex flex-wrap gap-2">
          {availableColumns.map((column) => (
            ...
            >
              {column}
              {selectedColumns.includes(column) && <X className="w-3 h-3 ml-1" />}
            </Button>
          ))}
        </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            {selectedColumns.map((column) => (
              <Badge key={column} variant="secondary" className="pl-2 pr-1 py-1 text-xs">
                {column}
                <button
        ...
        <div className="flex flex-col gap-3 animate-in fade-in-0 slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex items-center gap-2">
              {generatedFormula.isValid ? (
                ...
                <>
                  ...
                  <span className="text-xs font-medium text-destructive">Invalid Formula</span>
                </>
              )}
            </div>
          ...
          <div className="relative group">
            ...
            <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
              <Button variant="secondary" size="icon" className="h-7 w-7" onClick={handleCopy}>
      ...
      {error && !generatedFormula && (
        <div className="p-4 rounded-lg bg-destructive/10 border border-destructive/20 animate-in fade-in-0">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-xs font-medium text-destructive">Failed to generate formula</p>
