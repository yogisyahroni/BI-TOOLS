  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 px-3 py-2.5 border-b border-border/30 bg-gradient-to-r from-primary/5 to-transparent rounded-t-xl">
        ...
        {node.schema && (
          <Badge variant="outline" className="h-4 text-[8px] px-1 font-mono">
            {node.schema}
          </Badge>
        )}
      </div>
  ...
  return (
    ...
    >
      ...
      <circle
        ...
        cy={from.y}
        r={3.5}
        fill={isSelected ? "hsl(var(--primary))" : "hsl(var(--muted-foreground) / 0.4)"}
      />
      <circle
        cx={to.x}
        cy={to.y}
...
export function RelationshipEditor({
  ...
  const [linkingFrom, setLinkingFrom] = React.useState<{ table: string; column: string } | null>(
    null,
  );
  const [newRelType, setNewRelType] = React.useState<"one-to-one" | "one-to-many" | "many-to-many">(
    "one-to-many",
  );
  ...
  const handleColumnClick = (table: string, column: string) => {
    if (!linkingFrom) {
      ...
      const duplicate = rels.some(
        (r) =>
          (r.fromTable === newRel.fromTable &&
            r.fromColumn === newRel.fromColumn &&
            r.toTable === newRel.toTable &&
            r.toColumn === newRel.toColumn) ||
          (r.fromTable === newRel.toTable &&
            r.fromColumn === newRel.toColumn &&
            r.toTable === newRel.fromTable &&
            r.toColumn === newRel.fromColumn),
      );
  ...
  return (
    <Card
      className={cn(
        "flex flex-col overflow-hidden bg-card/50 backdrop-blur-xl border-border/40",
        className,
      )}
    >
      {/* Toolbar */}
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-border/30 bg-card/60">
        ...
        {linkingFrom && (
          <Badge
            variant="default"
            className="h-6 text-[10px] bg-primary/20 text-primary border-primary/30 animate-pulse"
          >
          ...
          <SelectContent>
            <SelectItem value="one-to-one" className="text-xs">
              <span className="flex items-center gap-1">
                <ArrowLeftRight className="w-3 h-3" /> One-to-One
              </span>
            </SelectItem>
            <SelectItem value="one-to-many" className="text-xs">
              <span className="flex items-center gap-1">
                <ArrowRight className="w-3 h-3" /> One-to-Many
              </span>
            </SelectItem>
            <SelectItem value="many-to-many" className="text-xs">
              <span className="flex items-center gap-1">
                <ArrowLeftRight className="w-3 h-3" /> Many-to-Many
              </span>
            </SelectItem>
          </SelectContent>
        </Select>
      ...
      <div className="flex flex-1 min-h-0">
          ...
          <ScrollArea className="flex-1">
            <div className="px-2 pb-2 space-y-0.5">
              {(tableSearch
                ...
                const relCount = rels.filter(
                  (r) => r.fromTable === t.name || r.toTable === t.name,
                ).length;
                return (
                  <div
                    key={t.name}
        ...
        >
          ...
          >
            {/* SVG Lines */}
            <svg
              className="absolute inset-0 w-[4000px] h-[4000px] pointer-events-none"
              style={{ zIndex: 1 }}
            >
              <g style={{ pointerEvents: "auto" }}>{lines}</g>
            </svg>
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center space-y-2">
                ...
                <p className="text-[11px] text-muted-foreground/30">
                  Connect a data source to visualize relationships
                </p>
              </div>
            </div>
          )}
