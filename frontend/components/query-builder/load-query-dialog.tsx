...
import { Search, FolderOpen, Calendar, Tag, Database, AlertCircle, Loader2 } from "lucide-react";
...
export function LoadQueryDialog({ open, onOpenChange, workspaceId, onLoad }: LoadQueryDialogProps) {
  ...
  const fetchQueries = async () => {
    ...
    try {
      const response = await fetchWithAuth(`/api/go/visual-queries?workspace_id=${workspaceId}`);
          ...
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-sm text-muted-foreground">
                {searchQuery ? "No queries match your search" : "No saved queries found"}
              </p>
              {searchQuery && (
                <Button
                  variant="link"
                ...
                >
                  ...
                  <div className="flex items-center gap-4 text-xs text-muted-foreground">
                    <div className="flex items-center gap-1">
                      ...
                      <span>
                        {query.config.tables.length} tables, {query.config.joins.length} joins
                      </span>
                    </div>
                    <div className="flex items-center gap-1">
                      <Calendar className="h-3 w-3" />
                      <span>{format(new Date(query.updatedAt), "MMM d, yyyy")}</span>
                    </div>
                  </div>
                    ...
                    <div className="mt-3 pt-3 border-t space-y-2">
                      ...
                      <div className="grid grid-cols-3 gap-2 text-xs">
                        <div>
                          ...
                          <span className="font-medium">{query.config.columns.length}</span>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <span className="font-medium">{query.config.aggregations.length}</span>
                        </div>
                        <div>
                          <span className="text-muted-foreground">Group By:</span>{" "}
                          <span className="font-medium">{query.config.groupBy.length}</span>
                        </div>
                        <div>
                          <span className="text-muted-foreground">Limit:</span>{" "}
                          <span className="font-medium">{query.config.limit}</span>
                        </div>
                      </div>
                    </div>
                  )}
