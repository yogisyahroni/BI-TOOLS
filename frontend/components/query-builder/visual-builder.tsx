...
interface VisualBuilderProps {
  connectionId: string;
  tables: TableSchema[];
  onQueryChange: (config: { tables: TableNode[]; joins: JoinEdge[] }) => void;
}

export function VisualBuilder({ connectionId, tables, onQueryChange }: VisualBuilderProps) {
  // State
  ...
  const handleRemoveTable = (tableId: string) => {
    ...
    setJoins(joins.filter((j) => j.fromTable !== table.tableName && j.toTable !== table.tableName));
  ...
  const handleUpdateJoinType = (
    ...
    setJoins(joins.map((j) => (j.id === joinId ? { ...j, joinType: newType } : j)));
  };
                ...
                {tables.length === 0 ? (
                  ...
                  tables.map((table) => (
                    <DraggableTableCard
                      key={table.name}
                      table={table}
                      isSelected={selectedTables.some((t) => t.tableName === table.name)}
                    />
                  ))
                )}
              </div>
              ...
              {selectedTables.length === 0 ? (
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center space-y-2">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Drag tables from the left panel to start building your query
                    </p>
                  </div>
                </div>
              ) : (
                <div className="relative w-full h-full min-h-[400px]">
                          ...
                          <CardHeader className="pb-2 bg-primary/5">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="font-semibold text-sm">{tableSchema.name}</span>
                              </div>
                              <Button
                                variant="ghost"
                                size="sm"
                  ...
                  {joins.length > 0 && (
                    ...
                    >
                      {joins.map((join) => {
                        const fromNode = selectedTables.find((t) => t.tableName === join.fromTable);
                        const toNode = selectedTables.find((t) => t.tableName === join.toTable);
                        ...
                        return (
                          <g key={join.id}>
                            <line
                              ...
                              strokeWidth="2"
                              strokeDasharray={join.joinType === "LEFT" ? "5,5" : ""}
                              opacity="0.5"
                            />
                            <circle
                              cx={(x1 + x2) / 2}
            ...
            {activeId && draggedTable ? (
              <Card className="w-64 shadow-xl opacity-50">
                <CardHeader className="pb-2">
                  <div className="flex items-center gap-2">
                    <Table className="h-4 w-4" />
                    <span className="font-semibold text-sm">{draggedTable.name}</span>
                  </div>
                </CardHeader>
              </Card>
            ) : null}
                  ...
                  <div className="p-4 space-y-3">
                    {joins.length === 0 ? (
                      <div className="text-center py-8 space-y-2">
                        ...
                        <p className="text-sm text-muted-foreground">No joins defined yet</p>
                            ...
                            <div className="flex items-start justify-between">
                              <div className="flex-1 space-y-1">
                                <div className="text-xs font-mono">
                                  <span className="font-semibold">{join.fromTable}</span>.
                                  {join.fromColumn}
                                </div>
                            ...
                            <Select
                              ...
                              onValueChange={(value) =>
                                handleUpdateJoinType(
                                  join.id,
                                  value as "INNER" | "LEFT" | "RIGHT" | "FULL OUTER",
                                )
                              }
                            >
                              ...
                              <SelectContent>
                                ...
                                <SelectItem value="FULL OUTER">FULL OUTER JOIN</SelectItem>
                              </SelectContent>
                            </Select>
                          </div>
                        </Card>
                ...
                <ScrollArea className="h-[calc(100vh-300px)]">
                  <div className="p-4 space-y-3">
                    {selectedTables.length < 2 ? (
                      ...
                      <div className="text-center py-8 space-y-2">
                        <AlertCircle className="h-8 w-8 mx-auto text-muted-foreground/50" />
                        <p className="text-sm text-muted-foreground">No join suggestions found</p>
                        <p className="text-xs text-muted-foreground">Define joins manually</p>
                      </div>
                    ) : (
                        ...
                        <Card key={idx} className="p-3">
                          <div className="space-y-2">
                            ...
                            <div className="text-xs font-mono space-y-1">
                              <div>
                                <span className="font-semibold">{suggestion.fromTable}</span>.
                                {suggestion.fromColumn}
                              </div>
                              ...
                              <div>
                                <span className="font-semibold">{suggestion.toTable}</span>.
                                {suggestion.toColumn}
                              </div>
                            </div>
...
function DraggableTableCard({ table, isSelected }: { table: TableSchema; isSelected: boolean }) {
      ...
      <CardContent className="p-3">
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground">{table.columns.length} column(s)</p>
          </div>
          {isSelected && (
