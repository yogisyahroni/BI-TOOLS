...
import { ChevronLeft, ChevronRight, Home, RotateCcw, ExternalLink } from "lucide-react";
...
export function DrillNavigation({
  ...
  const breadcrumbTrail = useMemo(() => getBreadcrumbTrail(drillPath), [drillPath]);

  const canGoBack = useMemo(() => canDrillUp(drillPath), [drillPath]);

  const canGoForward = useMemo(() => canDrillDown(drillPath), [drillPath]);
          ...
          {breadcrumbTrail.map((level, index) => {
            ...
            return (
              <React.Fragment key={level.id}>
                <BreadcrumbItem>
                  {isLast ? (
                    <BreadcrumbPage className={cn("flex items-center gap-2", compact && "text-xs")}>
                      <span className="font-medium">{level.name}</span>
                      {showValues && level.value && (
                        <Badge
                          variant="secondary"
                          className={cn("font-normal", compact && "text-xs px-1.5 py-0")}
                        >
                          {String(level.value)}
                        </Badge>
                      )}
                      {isCurrent && (
                        <Badge
                          variant="default"
                          className={cn("ml-1", compact && "text-xs px-1.5 py-0")}
                        >
                          Current
                        </Badge>
                      )}
                    ...
                    >
                      {index === 0 && <Home className={cn("h-3.5 w-3.5", compact && "h-3 w-3")} />}
                      <span>{level.name}</span>
                      {showValues && level.value && (
                        <Badge
                          variant="outline"
                          className={cn("font-normal", compact && "text-xs px-1 py-0")}
                        >
                          {String(level.value)}
                        </Badge>
                      )}
                ...
                {!isLast && <BreadcrumbSeparator className={compact ? "text-xs" : ""} />}
              </React.Fragment>
            );
          })}
        </BreadcrumbList>
...
export function DrillBreadcrumb({
  ...
  showValues = true,
  className,
}: Pick<DrillNavigationProps, "drillPath" | "onNavigateToLevel" | "showValues" | "className">) {
  return (
    <DrillNavigation
      drillPath={drillPath}
