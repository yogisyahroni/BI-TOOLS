...
export function AiTextWidget({ title, description, initialContent }: AiTextWidgetProps) {
  const [content, setContent] = useState<string>(
    initialContent || "AI summary will appear here. Click regenerate to fetch new insights.",
  );
  ...
  const handleRegenerate = async () => {
    ...
    setTimeout(() => {
      setContent(
        "Here is a fresh AI-generated summary of your dashboard data. Sales are up 15% compared to last month, driven primarily by strong performance in the North America region.",
      );
      setIsLoading(false);
    }, 1500);
  };
