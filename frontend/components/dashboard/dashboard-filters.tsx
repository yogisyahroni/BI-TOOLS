  ...
  return (
    <div className="flex flex-wrap items-center gap-2 py-3 px-6 border-b border-border bg-card/50">
      ...
      {filters.map((filter) => (
        <div
          key={filter.id}
          className="flex items-center gap-1 bg-background border border-border rounded-md pl-2 pr-1 py-1"
        >
          <span className="text-xs font-semibold text-muted-foreground mr-1 capitalize">
            {filter.name}:
          </span>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start">
                {["All", "Category A", "Category B", "Category C"].map((option) => (
                  <DropdownMenuItem
                    key={option}
                    onClick={() => onUpdateFilter(filter.id, option === "All" ? null : option)}
                  >
      ...
      <Popover>
        <PopoverTrigger asChild>
          <Button
            variant="outline"
            size="sm"
            className="h-8 gap-1.5 ml-2 bg-primary/5 border-primary/20 hover:bg-primary/10 text-primary"
          >
        ...
        <PopoverContent className="w-80 p-3" align="start">
          <div className="space-y-2">
            ...
            <p className="text-[10px] text-muted-foreground">
              Describe what you want to filter (e.g. "Sales from last month")
            </p>
            <div className="flex gap-2">
              <Input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") {
                    ...
                    onAIFilter?.(target.value);
                    // Close popover logic would need state, for now user can click out
                  }
                }}
                id="ai-filter-input"
              />
              <Button
                ...
                className="h-8 px-2"
                onClick={() => {
                  const input = document.getElementById("ai-filter-input") as HTMLInputElement;
                  if (input && input.value) onAIFilter?.(input.value);
                }}
              >
