...
const ChartVisualization = dynamic(
  () => import("@/components/chart-visualization").then((mod) => mod.ChartVisualization),
  {
    ssr: false,
    loading: () => (
...
export function DashboardCard({
  ...
  return (
    <>
      <Card
        id={`card-${card.id}`}
        className={`h-full flex flex-col overflow-hidden border-border/50 shadow-sm hover:shadow-md transition-shadow relative group ${className}`}
      >
        {/* Header */}
                        ...
                        onClick={() => {
                          try {
                            ExportService.exportDataToCSV(
                              card.data!,
                              `${card.title.replace(/\s+/g, "_")}_data.csv`,
                            );
