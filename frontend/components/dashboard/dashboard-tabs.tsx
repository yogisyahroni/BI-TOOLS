...
export function DashboardTabs({
  ...
  return (
    <div className={cn("w-full", className)}>
      ...
      {/* Add Tab Dialog */}
      <AddTabDialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen} onAdd={handleAddTab} />
      ...
      <DialogContent>
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            ...
            <DialogDescription>Update the tab name and description.</DialogDescription>
          </DialogHeader>
...
export function useDashboardTabs(initialTabs: DashboardTab[] = []) {
  ...
  const addTab = useCallback(
    (tab: Omit<DashboardTab, "id" | "order">) => {
  ...
  const renameTab = useCallback((tabId: string, newName: string, newDescription?: string) => {
    setTabs((prev) =>
      prev.map((t) => (t.id === tabId ? { ...t, name: newName, description: newDescription } : t)),
    );
  }, []);
