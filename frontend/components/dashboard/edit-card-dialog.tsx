...
export function EditCardDialog({ open, onOpenChange, card, onSave }: EditCardDialogProps) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const [clickAction, setClickAction] = useState<any>(
    card.visualizationConfig?.clickAction || "filter",
  );
  const [drillType, setDrillType] = useState<string>(
    card.visualizationConfig?.drillConfig?.type || "dashboard",
  );
  const [targetDashboardId, setTargetDashboardId] = useState(
    card.visualizationConfig?.drillConfig?.dashboardId || "",
  );
  const [targetUrl, setTargetUrl] = useState(card.visualizationConfig?.drillConfig?.url || "");
  const [openInNewTab, setOpenInNewTab] = useState(
    card.visualizationConfig?.drillConfig?.openInNewTab || false,
  );

  // Tooltip State
  const [tooltipTemplate, _setTooltipTemplate] = useState(
    card.visualizationConfig?.tooltipTemplate || "",
  );
  ...
  useEffect(() => {
    if (open && clickAction === "drill") {
      fetchWithAuth("/api/go/dashboards")
        .then((res) => res.json())
        .then((data) => {
                  ...
                  {drillType === "url" && (
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Use {"{{value}}"} or {"{{series}}"} as placeholders.
                      </p>
                    </div>
                  )}
