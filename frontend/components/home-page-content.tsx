...
export function HomePageContent() {
  ...
  const [_queryStatus, setQueryStatus] = useState<"draft" | "verified" | "deprecated" | undefined>(
    undefined,
  );
  ...
  const { activeConnection, fetchSchema, schema } = useConnections({
    autoFetch: true,
  });
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {showSchemaBrowser && (
          <div className="w-72 flex flex-col border-r border-border bg-card overflow-hidden flex-shrink-0">
            ...
            <div className="flex-1 overflow-y-auto">
              <SchemaBrowser onClose={() => setShowSchemaBrowser(false)} schema={schema} />
            </div>
          </div>
        )}
      </div>
