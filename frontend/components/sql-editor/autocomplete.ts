    ...
    return {
        provideCompletionItems: (model, position) => {
            ...
            if (context.afterFROM || context.afterJOIN) {
                ...
                addTableSuggestions(monacoInstance, suggestions, schemas, range);
            }

            if (context.afterSELECT || context.afterWHERE || context.afterON) {
                // Suggest columns and tables
                addColumnSuggestions(monacoInstance, suggestions, schemas, range, context.currentTable);
                addTableSuggestions(monacoInstance, suggestions, schemas, range);
            }
            ...
            // Always add keywords
            addKeywordSuggestions(monacoInstance, suggestions, range);

            // Always add data types (useful for CREATE TABLE, CAST, etc.)
            addDataTypeSuggestions(monacoInstance, suggestions, range);

            // Add function suggestions
            addFunctionSuggestions(monacoInstance, suggestions, range, context);
