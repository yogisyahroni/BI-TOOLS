...
export function ProviderList({ onEdit }: ProviderListProps) {
  const { providers, isLoading, deleteProvider, testProvider, isDeleting, isTesting } =
    useAIProviders();
        ...
        <TableBody>
          {providers.map((provider) => (
            <TableRow key={provider.id}>
              ...
              <TableCell>
                <code className="text-xs bg-muted px-2 py-1 rounded">{provider.model}</code>
              </TableCell>
              <TableCell>
                <Badge variant={provider.isActive ? "default" : "secondary"} className="gap-1">
                  {provider.isActive ? (
                    <>
