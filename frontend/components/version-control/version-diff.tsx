        ...
        {from && to && (
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div className="space-y-1">
              ...
              <div className="p-2 rounded bg-red-50 text-red-700 line-through">{from}</div>
            </div>
            <div className="space-y-1">
              <span className="text-xs text-muted-foreground">After</span>
              <div className="p-2 rounded bg-green-50 text-green-700">{to}</div>
            </div>
          </div>
        )}
      </div>
          ...
          <div key={index} className="p-3 rounded-lg border border-yellow-200 bg-yellow-50/50">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <div className="text-xs text-foreground">{change.after.title || "Untitled"}</div>
              </div>
            </div>
          </div>
        ))}
    ...
    return (
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="overview" className="space-y-4 mt-4">
          ...
          {/* Description change */}
          {diff.descChanged &&
            renderChangeIndicator(true, diff.descFrom, diff.descTo, "Description")}
        </TabsContent>
    ...
    return (
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="metadata" className="space-y-4 mt-4">
          {diff.aiPromptChanged &&
            renderChangeIndicator(true, diff.aiPromptFrom, diff.aiPromptTo, "AI Prompt")}
