      ...
      <div className="space-y-1">
        {colorSteps.map((step, index) => (
          <div key={index} className="flex items-center gap-2 text-xs">
            ...
            />
            <span className="text-gray-600 dark:text-gray-400">{formatMapNumber(step.value)}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    return (
      <div className={`relative ${className}`} style={{ height, width }}>
        <Alert variant="destructive">
          <AlertDescription>{error || "Invalid GeoJSON data"}</AlertDescription>
        </Alert>
      </div>
    );
  }
  ...
  if (Object.keys(data).length === 0) {
    return (
      <div className={`relative ${className}`} style={{ height, width }}>
        <Alert>
          <AlertDescription>
            No metric data provided for choropleth map. Map will display regions without color
            coding.
          </AlertDescription>
        </Alert>
        <div className="mt-2">
          <MapContainer
            ...
            style={{
              height: typeof height === "number" ? `${height - 60}px` : `calc(${height} - 60px)`,
              width: "100%",
              borderRadius: "0.5rem",
            }}
          >
  ...
  return (
    <div className={`relative ${className}`} style={{ height, width }}>
      ...
      {config.showLegend !== false && legendSteps.length > 0 && (
        <ChoroplethLegend colorSteps={legendSteps} title={config.dataColumn} />
      )}
