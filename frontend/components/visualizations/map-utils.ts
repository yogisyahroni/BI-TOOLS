 ...
 */
export function getColorForValue(value: number, colorScale: ColorScale): string {
  ...
  if (type === "diverging") {
    ...
    if (clampedValue < 0.5) {
      ...
      // Upper half
      const upperColorIndex =
        (clampedValue - 0.5) * 2 * (colors.length - middleIndex - 1) + middleIndex;
