  ...
  async function handleGeneratePreview() {
    ...
    try {
      ...
      const response = await fetchWithAuth(
        `/api/go/upload/analyze?fileType=${fileType}&action=preview`,
        {
          method: "POST",
          body: formData,
        },
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <StepIndicator
          ...
          completed={step === "preview" || step === "mapping"}
          label="Configure"
        />
        <ChevronRight className="h-4 w-4 text-muted-foreground" />
        <StepIndicator
          step={3}
          active={step === "preview" || step === "mapping"}
          completed={false}
        ...
        >
          ...
          <div className="flex flex-col items-center gap-4 text-center">
            ...
            <div className="flex gap-2">
              {acceptedFileTypes.includes("csv") && <Badge variant="outline">CSV</Badge>}
              {acceptedFileTypes.includes("excel") && <Badge variant="outline">Excel</Badge>}
              {acceptedFileTypes.includes("json") && <Badge variant="outline">JSON</Badge>}
            </div>
      ...
      {step === "configure" && file && (
        <Card className="p-6">
          <div className="space-y-4">
            ...
            {fileType === "csv" && <CSVOptions options={options} onUpdate={updateOption} />}
            {fileType === "excel" && (
              <ExcelOptions options={options} onUpdate={updateOption} preview={preview} />
            )}
            {fileType === "json" && <JSONOptions options={options} onUpdate={updateOption} />}
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-2">
          ...
          <Select value={options.delimiter} onValueChange={(value) => onUpdate("delimiter", value)}>
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
