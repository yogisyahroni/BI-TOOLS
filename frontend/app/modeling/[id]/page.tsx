          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              <Button
                ...
                size="sm"
                onClick={() => router.back()}
                className="h-6 px-0 hover:bg-transparent text-muted-foreground hover:text-foreground"
              >
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <div className="space-y-1">
                ...
                <CardDescription>Columns from the source table.</CardDescription>
              </div>
            </CardHeader>
            <CardContent>
              {model.dimensions?.length === 0 ? (
                <p className="text-muted-foreground text-sm">
                  No dimensions found. Use 'Refresh' to sync with database schema.
                </p>
              ) : (
                <Table>
                  <TableHeader>
                      ...
                      <TableRow key={dim.id} className={dim.isHidden ? "opacity-50" : ""}>
                        ...
                        <TableCell className="text-right">
                          <Button
                            ...
                            size="icon"
                            className="h-8 w-8"
                            onClick={() => toggleDimensionVisibility(dim)}
                          >
                            {dim.isHidden ? (
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <div className="space-y-1">
                ...
                <CardDescription>Define calculations to be computed on the fly.</CardDescription>
              </div>
            ...
            <CardContent>
              {model.metrics?.length === 0 ? (
                <div className="text-center py-12 text-muted-foreground bg-muted/10 rounded-xl border border-dashed">
                  ...
                  <Button variant="link" onClick={() => setShowCreateMetricDialog(true)}>
                    Create one now
                  </Button>
                </div>
              ) : (
                  ...
                  <TableBody>
                    {model.metrics?.map((metric) => (
                      <TableRow key={metric.id}>
                        <TableCell className="font-medium text-primary">{metric.name}</TableCell>
                        <TableCell>
