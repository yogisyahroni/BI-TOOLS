...
export default function GlossaryPage() {
  ...
  const { data: terms = [], isLoading: loading } = useQuery({
    ...
    queryFn: async () => {
      return await glossaryApi.listTerms();
    },
  });

  const createMutation = useMutation({
    mutationFn: (term: Partial<BusinessTerm>) =>
      glossaryApi.createTerm(term as Omit<BusinessTerm, "id">),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: (term: Partial<BusinessTerm>) => glossaryApi.updateTerm(term.id!, term),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["glossaryTerms"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => glossaryApi.deleteTerm(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["glossaryTerms"] });
  ...
  const handleSave = async () => {
    if (isEditing && currentTerm.id) {
      updateMutation.mutate(currentTerm);
    } else {
      createMutation.mutate(currentTerm);
    }
  };
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Define and manage standardized business terminology.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredTerms.map((term) => (
            <Card key={term.id} className="hover:shadow-md transition-shadow">
              ...
              <CardContent>
                <p className="text-sm text-muted-foreground mb-4 line-clamp-3">{term.definition}</p>
