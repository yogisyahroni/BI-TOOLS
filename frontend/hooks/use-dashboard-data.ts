...
export function useDashboardData(dashboard: Dashboard | null, options: UseDashboardDataOptions = {}) {
        ...
        try {
            ...
            const promises = queryCards.map(async (card) => {
                ...
                try {
                    ...
                    // 0. Apply Cross-Filters (WHERE wrapper) - Exclude self
                    if (options.crossFilters && options.crossFilters.length > 0) {
                        const applicableFilters = options.crossFilters.filter(f => f.sourceChartId !== card.id);
                        if (applicableFilters.length > 0) {
                            finalSql = applyFiltersToSql(finalSql, applicableFilters);
                        }
                    }
                    ...
                    if (options.globalFilters && Object.keys(options.globalFilters).length > 0) {
                        ...
                        // 2. Inject WHERE clauses (Legacy wrapper for sidebar filters)
    ...
    }, [dashboard?.id, JSON.stringify(options.globalFilters), JSON.stringify(options.crossFilters)]);
