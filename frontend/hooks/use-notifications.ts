...
export function useNotifications(limit = 20, offset = 0) {
    ...
    // Query: Get notifications (only when authenticated)
    const { data, isLoading, error } = useQuery({
        queryKey: ['notifications', limit, offset],
        queryFn: () => notificationApi.getNotifications(limit, offset),
        enabled: isAuthenticated,
        refetchOnWindowFocus: false, // Prevent refetch on tab switch when not authenticated
    });

    // Query: Get unread count (only when authenticated)
    ...
    // Connect to WebSocket for real-time updates via Context
    const { subscribe } = useWebSocketContext();

    // Subscribe to notifications
    useEffect(() => {
        if (isAuthenticated) {
            const unsubscribe = subscribe('notification', handleNotificationUpdate);
            return () => {
                unsubscribe();
            };
        }
    }, [isAuthenticated, subscribe, handleNotificationUpdate]);
