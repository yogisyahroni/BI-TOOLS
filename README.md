...
```bash
# Copy template (DO NOT commit the actual .env file)
cp backend/.env.example backend/.env.local
cp frontend/.env.example frontend/.env.local
# Edit with secure values
```

**⚠️ SECURITY WARNING**: Never commit `.env` files with actual secrets. Use environment variables in production.

**Required Environment Variables**:

```env
# Database (use environment variables, not hardcoded values)
DATABASE_URL=postgresql://user:YOUR_SECURE_PASSWORD@localhost:5432/insightengine

# Security - Generate secure values:
# JWT_SECRET: openssl rand -base64 32
# ENCRYPTION_KEY: openssl rand -base64 32
JWT_SECRET=CHANGE_THIS_TO_SECURE_RANDOM_STRING
ENCRYPTION_KEY=CHANGE_THIS_TO_32_BYTE_BASE64_ENCODED_SECRET
...
1. ✅ **Generate secure secrets**:
   ...
   # Use password manager or generate random
   ```
...
5. ✅ **Enable database SSL** (sslmode=require)
