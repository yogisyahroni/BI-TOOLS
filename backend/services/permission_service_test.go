...
func seedTestUser(t *testing.T, db *gorm.DB, userID string) *models.User {
	t.Helper()
	user := &models.User{
		ID:       userID,
...
func TestAssignRoleToUser_Success(t *testing.T) {
	...
	err = svc.AssignRoleToUser(user.ID, role.ID, "admin-001")
	assert.NoError(t, err)
...
func TestRemoveRoleFromUser_Success(t *testing.T) {
	...
	err = svc.RemoveRoleFromUser(user.ID, role.ID)
	assert.NoError(t, err)
...
func TestCheckPermission_HasPermission(t *testing.T) {
	...
	hasPermission, err := svc.CheckPermission(user.ID, "query:create")
	assert.NoError(t, err)
...
func TestCheckPermission_DoesNotHavePermission(t *testing.T) {
	...
	hasPermission, err := svc.CheckPermission(user.ID, "admin:manage_users")
	assert.NoError(t, err)
...
func TestGetUserPermissions_MultipleRoles(t *testing.T) {
	...
	// Assign both roles
	err = svc.AssignRoleToUser(user.ID, roleA.ID, "admin-001")
	require.NoError(t, err)
	err = svc.AssignRoleToUser(user.ID, roleB.ID, "admin-001")
	require.NoError(t, err)

	permissions, err := svc.GetUserPermissions(user.ID)
	assert.NoError(t, err)
